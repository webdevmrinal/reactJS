  ...
  return !resList ? (
    ...
    <div className="flex justify-between pt-7 mx-4 border-b border-gray-300">
      <span className="text-2xl font-medium">
        {String(resList?.data?.cards[index]?.data?.data?.totalOpenRestaurants) +
          " restaurants"}
      </span>
      <span className="font-light text-sm flex pb-2">
        {resList?.data?.sorts?.map((sorts) => {
          return <button className="mr-5">{sorts?.title}</button>;
        })}
        {/* <button className="font-medium underline underline-offset-[19px] mr-5">
          Relevence
        </button> */}
